...
只要每個 party 把手上的 share 加上 c, 這份新的 shares (c + p1, c + p2, c + p3) 就是一份 [c + p] 的 shares.<br>
(請自行用多項式 c + P 來證明)

(c * p1, c * p2, c * p3) 則是一份 [c * p] 的 shares.<br>
(請自行用多項式 c * P 來證明)
...
只要每個 party 把手上的兩個數字相加, 這份新的 shares (p1 + q1, p2 + q2, p3 + q3) 就是一份 [p + q] 的 shares.<br>
(請自行用多項式 P + Q 來證明)
...
上面的 [c + p] [c * p] [p + q] [ap + bq + cr + ds] 都不會提高穿過 shares 的多項式所需的 degree.

比方說, 假設 [p] [q] 都用 1 次直線就能穿過. 則算出來的 [p + q] 也只需要 1 次直線就能穿過, 不需要用二次曲線.
